...
export class LoginComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.activatedRoute.queryParams.subscribe(params => {
            ...
            if (params.code) {
                ...
                this.openMainPage(code, state); // пытаемся авторизоваться и открыть главную страницу с данными
    ...
    private showAuthWindow(): void {
        ...
        const params = [
            ...
            'redirect_uri=' + encodeURIComponent(environment.frontendURL), // ответ получаем на клиента, который уже отправит этот код в BFF и обменяет на токены
        ];
    ...
    private openMainPage(code, state: string): void {
        ...
        this.bffService.exchangeCodeToTokens(code).subscribe({
            next: (() => {

                // если запрос на получение токенов в BFF выполнился успешно -
                // значит токены будут сохранены в безопасные server-side куки (не считываются на клиенте и более безопасны) и будут автоматически
